	  ...
	  <div class="content">
		...
		<div class="day-views">
		  ...
		  <div class="day-view">
			...
			<button @click="nextSaturday" :disabled="currentSaturdayIndex === saturdays.length - 1">Próximo</button>
			<div v-if="saturdays.length">
			  <h3>{{ formatDate(saturdays[currentSaturdayIndex]) }}</h3>
			  <div v-for="sector in sectors" :key="sector.id">
				<label>{{ sector.name }}</label>
				<el-select v-model="schedule[saturdays[currentSaturdayIndex]][sector.id]" multiple placeholder="Selecione os trabalhadores">
				  <el-option
		  ...
		  <div class="day-view">
			...
			<div v-if="sundays.length">
			  ...
			  <div v-for="sector in sectors" :key="sector.id">
				<label>{{ sector.name }}</label>
				<el-select v-model="schedule[sundays[currentSundayIndex]][sector.id]" multiple placeholder="Selecione os trabalhadores">
				  <el-option
					v-for="worker in workers"
					:key="worker.id"
					:label="worker.name"
